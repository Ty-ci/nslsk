...
// candidates add slots to the `eventy` sheet, the copy switches from an offer to
// a concrete date.
const InviteBand = () => {
  const { content } = useSheetContent()
  const nextEvent = content.events.at(0)
      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-8 px-6 py-14 md:flex-row md:items-center md:justify-between md:p-16">
        <div className="max-w-xl">
          ...
          <p className="mt-4 font-heading text-3xl font-bold uppercase md:text-4xl">
            {nextEvent ? (
              <>
                Najbližšie sa vidíme {nextEvent.term} — {nextEvent.form || nextEvent.title}.
              </>
            ) : (
              <>
