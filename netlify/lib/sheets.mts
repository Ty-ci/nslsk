...
// `google-auth-library`: that package is CommonJS and `require`s child_process
// at load time, which an ESM function bundle can't do. One signed JWT and one
// token request is all a read-only Sheets call needs anyway.

import { createSign } from 'node:crypto'

import type { SheetContent, SheetEvent, SheetQuestion } from '../../src/sheetContent.ts'
